...
var SearchBoxComponent = /** @class */ (function () {
    ...
    SearchBoxComponent.prototype.checkAll = function (ev) {
        ...
        switch (value) {
            case "administrative":
                this.administrative.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "arts":
                this.arts.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "construction":
                this.construction.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "education":
                this.education.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "finance":
                this.finance.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "health":
                this.health.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "hotel":
                this.hotel.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "manufacturing":
                this.manufacturing.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "news":
                this.news.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "realEstate":
                this.arts.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "restaurant":
                this.restaurant.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "retail":
                this.retail.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "services":
                this.services.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "transportation":
                this.transportation.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "utilities":
                this.utilities.map(function (x) { return x.state = ev.target.checked; });
                break;
            case "wholeSale":
                this.wholeSale.map(function (x) { return x.state = ev.target.checked; });
                break;
            default:
                console.log('Error');
        }
